...
export function CTASection() {
  return (
    <section className="py-24 px-6 bg-page relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 mb-10">
          {benefits.map(b => (
            <span key={b} className="inline-flex items-center gap-1.5 text-sm text-secondary">
        ...
        <a
          href="/register"
          className="inline-flex items-center gap-2 px-8 py-3.5 text-sm font-semibold text-inverse bg-action-primary rounded-xl hover:bg-action-primary-hover active:scale-[0.98] transition-all shadow-lg shadow-action-primary/25"
        >
      ...
      <footer className="relative z-10 mt-24 pt-8 max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        <span className="text-xs text-muted">AI Interview Agent</span>
        <div className="flex items-center gap-6">
          <span className="text-xs text-tertiary">Built with React + FastAPI + LangGraph</span>
          <span className="text-xs text-tertiary">Groq LLM · Pinecone · SQLAlchemy</span>
        </div>
      </footer>
    </section>
  );
