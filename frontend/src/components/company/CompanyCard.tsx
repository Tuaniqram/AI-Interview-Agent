...
export function CompanyCard({ company, docCount = 0, sessionCount = 0, avgScore }: CompanyCardProps) {
  const navigate = useNavigate();

  return (
    <Card hover className="cursor-pointer" onClick={() => navigate(`/companies/${company.id}`)}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-primary truncate">{company.name}</h3>
          {company.website && (
            <p className="text-xs text-muted truncate mt-0.5">{company.website}</p>
          )}
          {company.description && (
            <p className="text-xs text-secondary mt-1 line-clamp-2">{company.description}</p>
          )}
