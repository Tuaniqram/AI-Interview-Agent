...
export function DepartmentForm({ open, initial, onSave, onCancel, saving }: DepartmentFormProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay">
      <div className="bg-elevated rounded-xl shadow-lg max-w-lg w-full mx-4 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="font-semibold text-primary">{initial ? 'Edit Department' : 'Add Department'}</h3>
          <button onClick={onCancel} className="text-muted hover:text-secondary"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-secondary mb-1.5">Department Name *</label>
